...
void construct_esp(char *file_name, void **esp) {
  ...
  token = strtok_r(stored_file_name, " ", &last); //extract program's name
  strlcpy(stored_file_name, file_name, strlen(file_name) + 1); //create a new thread to execute stored_file_name
  argc = 0;
